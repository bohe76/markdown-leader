...
function runtimeNotices() {
  ...
  function visit(name: string, importer: string) {
    ...
    // mathjax-newcm의 npm 배포본은 Apache-2.0 파일을 누락했으므로 동일 프로젝트의 원문을 사용한다.
    if (!licenseFiles.length && metadata.name === '@mathjax/mathjax-newcm-font' && metadata.license === 'Apache-2.0') {
      licenseFiles.push(readFileSync(resolve(packageDirectory('mathjax', root), 'LICENSE'), 'utf8').trim())
    }
  ...
  plugins: [{
    ...
    async writeBundle() {
      ...
      if (contentSource.includes('markdown-math')) {
        const mathjaxDirectory = packageDirectory('mathjax', root)
        const fontDirectory = packageDirectory('@mathjax/mathjax-newcm-font', root)
        const mathjaxOutput = resolve(output, 'renderers/mathjax')
        mkdirSync(resolve(mathjaxOutput, 'ui'), { recursive: true })
        mkdirSync(resolve(mathjaxOutput, 'input/tex'), { recursive: true })
        copyFileSync(resolve(source, 'content/mathjax-config.js'), resolve(mathjaxOutput, 'config.js'))
        copyFileSync(resolve(source, 'content/mathjax-bridge.js'), resolve(mathjaxOutput, 'bridge.js'))
        copyFileSync(resolve(mathjaxDirectory, 'tex-chtml.js'), resolve(mathjaxOutput, 'tex-chtml.js'))
        copyFileSync(resolve(mathjaxDirectory, 'ui/safe.js'), resolve(mathjaxOutput, 'ui/safe.js'))
        cpSync(resolve(mathjaxDirectory, 'input/tex/extensions'), resolve(mathjaxOutput, 'input/tex/extensions'), { recursive: true })
        cpSync(resolve(fontDirectory, 'chtml/woff2'), resolve(output, 'assets/mathjax/chtml/woff2'), { recursive: true })
        cpSync(resolve(fontDirectory, 'chtml/dynamic'), resolve(output, 'assets/mathjax/chtml/dynamic'), { recursive: true })
      }
