...
export function createReviewUI({ document, window, content, t, onSave }) {
  ...
  function renderCard(note, draft) {
      ...
      if (editing) {
        ...
        input.rows = 5
        input.addEventListener('input', () => { draft.text = input.value; save.disabled = !input.value.trim() || saving.has(current.id) })
        ...
        save.disabled = !draft.text.trim() || saving.has(current.id)
        ...
        storageHint.textContent = t('reviewOriginalHint')
      ...
      const edit = button('reviewEdit', () => { drafts.set(current.id, { ...note }); draftOriginals.set(current.id, { id: note.id, version: noteVersion(note) }); renderPanel(); list.querySelector('textarea')?.focus() })
      const remove = button('reviewDelete', () => persist(notes.filter(item => item.id !== note.id)))
      edit.disabled = remove.disabled = saving.has(current.id)
  ...
  async function persist(nextNotes, draft) {
    ...
    try {
      ...
      if (draft && drafts.get(owner) === draft) {
        drafts.delete(owner)
        draftOriginals.delete(owner)
      }
      if (current?.id === owner && (current.raw === expectedRaw || current.raw === raw)) {
        current.raw = raw
  ...
  return { element, clear, forget, setDocument, refreshAnchors, isOpen: () => !panel.hidden, toggle() { panel.hidden = !panel.hidden; if (!panel.hidden) renderPanel() } }
}
