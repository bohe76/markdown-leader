export function createDocumentTabStrip({ document, t, onSelect, onClose, onAdd, onPin = (_id) => {}, onFavorite = (_id) => {}, onReorder = (_ids) => {} }) {
  const items = new Map()
  let order = []
  let drag = null
  ...
  function updateOverflow() {
    ...
    const available = Math.max(0, element.clientWidth - addButton.offsetWidth)
    list.style.width = `${Math.min(items.size * 180, available)}px`
    const overflow = items.size * 180 > available
  ...
  function reveal(item) {
    if (!item) return
    const left = item.wrapper.offsetLeft
  ...
  function add({ id, title, description }) {
    ...
    wrapper.append(tab, pin, favoriteButton, close)
    items.set(id, { wrapper, tab, close, pin, favoriteButton, pinned: false, favorite: false })
    order.push(id)
    tab.draggable = true
