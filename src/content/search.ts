...
function isExcluded(node: Text, root: HTMLElement): boolean {
  for (let element = node.parentElement; element && element !== root; element = element.parentElement) {
    if (EXCLUDED_TAGS.has(element.tagName) || element.matches('mjx-container, .ml-math[data-math-state="pending"], .ml-diagram-output, .ml-diagram-stage, .ml-pdf-repeat')) return true
  }
