...
.ml-pdf-content :is(th, td).ml-pdf-nowrap { white-space: nowrap; overflow-wrap: normal; }
.ml-pdf-content :is(th, td).ml-pdf-nowrap code { white-space: nowrap; overflow-wrap: normal; }
...
.ml-pdf-content .ml-math-block > mjx-container[display="true"] { overflow: visible; }
...
const COMPACT_TABLE_TEXT_LIMIT = 20
...
function breakPositions(element, granularity = 'word') {
  ...
  const atomic = 'svg,img,input,.ml-math,mjx-container,br'
...
function smallestMainText(node) {
  ...
  while ((text = walker.nextNode())) {
    ...
    if (!text.data.replace(/[\s\p{Cf}]/gu, '') || parent.closest('mjx-assistive-mml,mjx-script,script,style,details,title,desc,defs')) continue
...
export function fitTableWrapping(table) {
  const doc = table.ownerDocument
  const generatedColumns = table.querySelector(':scope > colgroup[data-ml-generated-columns]')
  if (generatedColumns) {
  ...
  function compactCellWidth(cell) {
    const range = doc.createRange()
    range.selectNodeContents(cell)
    const bounds = [range.getBoundingClientRect(), ...[...cell.children].map(child => child.getBoundingClientRect())]
      .filter(rect => rect.width > 0)
    const contentWidth = bounds.length ? Math.max(...bounds.map(rect => rect.right)) - Math.min(...bounds.map(rect => rect.left)) : 0
    const cellStyle = doc.defaultView.getComputedStyle(cell)
    return contentWidth + parseFloat(cellStyle.paddingLeft) + parseFloat(cellStyle.paddingRight)
      + parseFloat(cellStyle.borderLeftWidth) + parseFloat(cellStyle.borderRightWidth) + 12
  }
  for (const cell of cells) {
    const text = cell.textContent.replace(/\s+/g, ' ').trim()
    if (!text || [...text].length > COMPACT_TABLE_TEXT_LIMIT || cell.querySelector('br,img,svg,input,table,.ml-math')) continue
    cell.classList.add('ml-pdf-compact', 'ml-pdf-nowrap')
    if (compactCellWidth(cell) > protectionLimit) {
      cell.classList.remove('ml-pdf-nowrap')
      continue
    }
    const atomic = !/\s/u.test(text)
    // 본문 식별자 → 헤더 식별자 → 본문 구 → 헤더 구 순서로 한 줄 보호를 유지한다.
    const priority = atomic ? (cell.tagName === 'TD' ? 3 : 2) : (cell.tagName === 'TD' ? 2 : 1)
    const key = `${cell.cellIndex}:${priority}`
    const group = groups.get(key) || { cells: [], priority, score: 0 }
    group.cells.push(cell)
    group.score = Math.max(group.score, [...text].length)
    groups.set(key, group)
  }
  const overflows = () => Math.max(table.scrollWidth, table.getBoundingClientRect().width) > available + 1
  const removable = [...groups.values()].sort((left, right) => left.priority - right.priority || right.score - left.score)
  if (!wideTable) {
    while (overflows() && removable.length) {
  ...
  function minimumWidths() {
    ...
    for (const cell of cells) {
      if (cell.classList.contains('ml-pdf-nowrap')) {
        protectedWidths[cell.cellIndex] = Math.max(protectedWidths[cell.cellIndex], compactCellWidth(cell))
      }
    }
    return protectedWidths.map(width => Math.max(absoluteMinimum, width))
  }
  let widths = minimumWidths()
  let remaining = available - widths.reduce((sum, width) => sum + width, 0)
  while (remaining < 0 && removable.length) {
    for (const cell of removable.shift().cells) cell.classList.remove('ml-pdf-nowrap')
    widths = minimumWidths()
    remaining = available - widths.reduce((sum, width) => sum + width, 0)
  }
  if (remaining < 0) return
  function flowSample(cell) {
    const cellStyle = doc.defaultView.getComputedStyle(cell)
    const chrome = parseFloat(cellStyle.paddingLeft) + parseFloat(cellStyle.paddingRight)
      + parseFloat(cellStyle.borderLeftWidth) + parseFloat(cellStyle.borderRightWidth)
    let content = 0
    const walker = doc.createTreeWalker(cell, 4)
    let textNode
    while ((textNode = walker.nextNode())) {
      if (!textNode.data || textNode.parentElement?.closest('svg,.ml-math,script,style')) continue
      const range = doc.createRange()
      range.selectNodeContents(textNode)
      content += [...range.getClientRects()].reduce((sum, rect) => sum + rect.width, 0)
    }
    for (const atom of cell.querySelectorAll('img,svg,input,.ml-math')) {
      if (atom.closest('.ml-math') !== atom && atom.closest('.ml-math')) continue
      content += atom.getBoundingClientRect().width
    }
    for (const inline of cell.querySelectorAll('code')) {
      const inlineStyle = doc.defaultView.getComputedStyle(inline)
      content += parseFloat(inlineStyle.paddingLeft) + parseFloat(inlineStyle.paddingRight)
        + parseFloat(inlineStyle.borderLeftWidth) + parseFloat(inlineStyle.borderRightWidth)
    }
    return { content: Math.max(1, content), chrome, weight: cell.tagName === 'TH' ? 0.75 : 1 }
  }
  const samples = Array.from({ length: columnCount }, () => [])
  for (const cell of cells) samples[cell.cellIndex].push(flowSample(cell))
  const lineCount = (sample, width) => Math.max(1, Math.ceil(sample.content / Math.max(1, width - sample.chrome - 2)))
  const lineCost = (index, width) => samples[index].reduce((sum, sample) => sum + lineCount(sample, width) * sample.weight, 0)
  const candidates = samples.map((columnSamples, index) => {
    const values = new Set([Math.max(widths[index], readableMinimum)])
    for (const sample of columnSamples) {
      const startLines = Math.min(16, lineCount(sample, widths[index]))
      for (let lines = startLines - 1; lines >= 1; lines--) values.add(sample.chrome + sample.content / lines + 2)
    }
    return [...values].filter(width => width > widths[index] + 0.5).sort((left, right) => left - right)
  })
  while (remaining > 0.5) {
    let choice = null
    for (let index = 0; index < columnCount; index++) {
      const currentCost = lineCost(index, widths[index])
      for (const target of candidates[index]) {
        const delta = target - widths[index]
        if (delta > remaining + 0.01) break
        const benefit = currentCost - lineCost(index, target)
        if (benefit <= 0) continue
        const efficiency = benefit / delta
        if (!choice || efficiency > choice.efficiency + 1e-9
          || (Math.abs(efficiency - choice.efficiency) <= 1e-9 && benefit > choice.benefit)) {
          choice = { index, target, benefit, efficiency }
        }
      }
    }
    if (!choice) break
    remaining -= choice.target - widths[choice.index]
    widths[choice.index] = choice.target
  }
  const fittedWidth = widths.reduce((sum, width) => sum + width, 0)
  const columns = doc.createElement('colgroup')
  columns.dataset.mlGeneratedColumns = ''
...
export async function paginateDocument(source, pages, { isCurrent = () => Boolean(true), errorMessage = 'Page layout failed' } = {}) {
  ...
  function fitAtomicWidth(node) {
    ...
    const visual = node.matches('.ml-math') ? node.querySelector('mjx-container[display="true"]') || node : node
  ...
  async function place(node, chain = []) {
    ...
    target.append(node)
    if (node.nodeType === 1) {
      if (node.tagName === 'TABLE') fitTableWrapping(node)
      for (const table of node.querySelectorAll('table')) fitTableWrapping(table)
